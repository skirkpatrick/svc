...
//Largely taken from the official Golang sha512 test
import (
    "os"
    "testing"
)
...
var test = []sha512Test{
    ...
    {"be40ac85b542c684f8eb77a721d690b5580bb105a3a9a2bf873ff7bd84040d1193ffebe0218470ed41793ef8b5706660b6dbd2da763ef95cd629b763fd9b4783", "#include <iostream>\n#include \"hellow_world.cpp\"\n\nint main(int argc, char* argv[])\n{\n\tstd::cout << \"Hello, world!\" << std::endl;\n}"},
}
...
func TestCrypto(t *testing.T) {
    for i := 0; i < len(test); i++ {
        ...
        _, err = f.WriteString(test[i].in)
        if err != nil { panic(err) }
        err = f.Close()
        if err != nil { panic(err) }
        s, err := SHA512(testfile)
        if err != nil { panic(err) }
        if s != test[i].out {
            t.Fatalf("SHA512(%s) = %s want %s", test[i].in, s, test[i].out)
        }
    }
